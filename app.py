...
from utils.analyze_candidate import calculate_overall_score

MAX_QUESTIONS = 5

load_dotenv()
print("Loaded MISTRAL_API_KEY:", os.getenv("MISTRAL_API_KEY"))
...
def display_final_results():
    ...
    if (
        not st.session_state.get("show_final_results")
        or not st.session_state.get("conversations")
    ):
        return

    with st.spinner("Calculating final score..."):
        # Use calculate_overall_score instead of get_overall_evaluation_score
        final_score, rating = calculate_overall_score(st.session_state["conversations"])
    ...
    st.markdown(f"**Overall Score:** {final_score:.2f}/10 ({rating})")

    # Show detailed summary
    st.subheader("Detailed Interview Summary")
    for i, conv in enumerate(st.session_state["conversations"], 1):
        score = conv.get("Evaluation", 0)
        with st.expander(f"Question {i} (Score: {score}/10)", expanded=False):
            st.write(f"**Q:** {conv.get('Question', '')}")
            st.write(f"**A:** {conv.get('Candidate Answer', '')}")
            st.write(f"**Feedback:** {conv.get('Feedback', 'No feedback available')}")
